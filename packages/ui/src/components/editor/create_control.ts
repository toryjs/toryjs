...
export function editorProps(element: FormDataSet, props: FormComponentProps) {
  ...
  const onClick = React.useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      e.stopPropagation();
      context.editor.state.setElement(element, context.editor.schema);
    },
    [context.editor.schema, context.editor.state, element]
  );
  ...
  const onMouseOver = React.useCallback(
    (e: React.ChangeEvent<HTMLDivElement>) => {
      ...
      while (el.elements && el.elements.length && el.elements[0]) {
        let control = context.editor.editorCatalogue.components[el.elements[0].control];
      ...
      while (el != null && el.control) {
        ...
        child.onclick = ((e: React.MouseEvent<HTMLDivElement>) => {
          e.stopPropagation();
          context.editor.state.setElement(current, context.editor.schema);
        }) as any;
    ...
    [
      context.editor.editorCatalogue.components,
