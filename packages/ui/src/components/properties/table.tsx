...
import { DragDrop, Processor } from '../../editor/drag_drop';
...
function dragStart(index: number, context: EditorContextType) {
...
const TableRow: React.FC<RowProps> = props => {
  ...
  return (
    <div className={styledTableRow(context.theme)} onDragStart={onDragStart}>
      ...
      {!props.readOnly && (
        <div className={styles.tableRowAuto}>
          <button className={addButton(context.theme)} onClick={deleteRow} value={props.index}>
            x
          </button>
        </div>
      )}
...
Red - Handled by a function`}
    ...
    >
      ...
      {!controlProps.view || (controlProps.view === 'value' && list.length > 0) ? (
        <>
          <div className={celled(context.theme)} {...drag.props()}>
            {list.map((row, i) =>
          ...
          <ErrorView owner={props.owner} source={prop(formElement)} pointing="left" />
        </>
      ) : (
        undefined
      )}
