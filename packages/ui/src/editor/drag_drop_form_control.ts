...
export function dragEndHandler(context: EditorContextType) {
  return function() {
...
export function dragDropHandler(context: EditorContextType, item: FormDataSet) {
  return function(to: number, position: string) {
    const dragItem = context.dragItem;
    if (dragItem === null) {
      return;
    }
