...
import { DataSet, FormElement, Schema, Option, FormComponentProps, Handlers } from '@toryjs/form';
import { Dropdown } from 'semantic-ui-react';
import { getValue, Context, ContextType, propName } from '@toryjs/ui';
...
const MapRow: React.FC<RowProps> = (props: RowProps) => {
  ...
  const editorItem = context.editor.editorCatalogue.components[owner.getValue('control')];
  let schema: Schema;
  ...
  return (
    <div
      className={names(styledTableRow(context.editor.theme), { invalid: error })}
      onClick={e => {
...
export class Map extends React.Component<FormComponentProps> {
  ...
  onClear = () => {
    this.props.owner.setValue(propName(this.props.formElement), '');
  };

  addRow = () => {
    const source = propName(this.props.formElement);
  ...
  render() {
    ...
    return (
      ...
      >
        <div className={celled(this.context.editor.theme)}>
